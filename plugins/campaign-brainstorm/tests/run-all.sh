#!/usr/bin/env bash
# campaign-brainstorm test harness — structural assertions only.
set -uo pipefail
...
run_test "SKILL.md exists" "[ -f \"$PLUGIN_DIR/skills/campaign-brainstorm/SKILL.md\" ]"
...
for ref in brief-schema framework-layers martech-native-map workshop-questions smart-mode-checklist common-risks; do
    run_test "$ref.md exists" "[ -f \"$PLUGIN_DIR/skills/campaign-brainstorm/references/$ref.md\" ]"
done
...
echo ""
echo "=== Frontmatter ==="
SKILL="$PLUGIN_DIR/skills/campaign-brainstorm/SKILL.md"
run_test "SKILL.md has frontmatter name" "head -10 \"$SKILL\" | grep -q '^name: campaign-brainstorm'"
...
SCHEMA="$PLUGIN_DIR/skills/campaign-brainstorm/references/brief-schema.md"
...
MARTECH="$PLUGIN_DIR/skills/campaign-brainstorm/references/martech-native-map.md"
